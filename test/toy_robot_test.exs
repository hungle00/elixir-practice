defmodule ToyRobotTest do
  ...
  test "places the Toy Robot on the table" do
    {:ok, position } = ToyRobot.place(1, 1, :west)
    assert position == %ToyRobot.Position{facing: :west, x: 1, y: 1}
  end
  ...
  test "rotating the robot 3 times to the right is the same as rotating it to the left" do
    {:ok, robot} = ToyRobot.place(0, 0, :north)
    right_position = robot |> ToyRobot.right |> ToyRobot.right
                    |> ToyRobot.right |> ToyRobot.report
